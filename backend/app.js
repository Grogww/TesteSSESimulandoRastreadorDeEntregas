...
const STAGES = [
  { status: "pedido_recebido", label: "Pedido recebido", detail: "Seu pedido foi confirmado e está sendo processado." },
  { status: "em_separacao", label: "Em separação", detail: "Os itens estão sendo separados no centro de distribuição." },
  { status: "saiu_para_entrega", label: "Saiu para entrega", detail: "O pacote saiu do centro de distribuição rumo ao destino." },
  { status: "em_transito", label: "Em trânsito", detail: "O entregador está a caminho do endereço de entrega." },
  { status: "entregue", label: "Entregue", detail: "O pacote foi entregue com sucesso! Obrigado pela preferência." },
];
...
function advanceDelivery(deliveryId) {
  ...
  if (delivery.currentStage < STAGES.length - 1) {
    ...
    setTimeout(() => advanceDelivery(deliveryId), delay).unref();
  }
}
...
app.post("/api/deliveries", (_req, res) => {
  ...
  const delay = 4000 + Math.random() * 3000;
  setTimeout(() => advanceDelivery(id), delay).unref();
